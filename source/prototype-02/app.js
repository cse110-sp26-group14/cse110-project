...
window.onclick = (e) => { if (e.target === modalHost.querySelector('.modal-overlay')) modalHost.classList.add('hidden'); };

// Daily Check-In Modal Implementation
document.getElementById('btn-daily-checkin').onclick = () => {
    showModal('Daily Check-In', `
...
document.getElementById('btn-create-issue').onclick = () => {
    ...
    document.getElementById('issue-form').onsubmit = (e) => {
        ...
        if (window.location.hash === '#dashboard' || window.location.hash === '#issues') {
            router.handleRoute(); 
        }
    };
};
...
store.subscribe('issuesChanged', () => {
    if (window.location.hash === '#issues' || window.location.hash === '#dashboard') router.handleRoute();
});
